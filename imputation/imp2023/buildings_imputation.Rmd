...
* imputation/data2018/parcels.csv (optionally)
...
Set the name of the buildings file (`bld.file.name`) and parcels file (`pcl.file.name`), if the resulting dataset should be written into a file (`save.as.csv`) or/and mysql (`save.into.mysql`), the name of the resulting buildings dataset (`out.buildings.name`) and if parcels should be joined with 2018 parcels to impute missing land use type (`join.with.2018parcels`):
...
if(join.with.2018parcels) {
    # read 2018 parcels table
    pcl18.file.name <- file.path(data.dir, "ref2018", "parcels.csv")
    pcl2018 <- fread(pcl18.file.name)
    pcl2018[, parcel_id_2018 := parcel_id]
}
```

The buildings table `r bld.file.name` has `r format(nrow(bld.raw), big.mark =",")` rows. The parcels table `r pcl.file.name` has `r format(nrow(pcl), big.mark =",")` rows. `r if(join.with.2018parcels){paste("2018 parcels table (file ", pcl18.file.name, ") has been loaded and has ", format(nrow(pcl2018), big.mark =","), " rows.")}`
...
#### Impute land use types using 2018 data


```{r, eval=join.with.2018parcels, echo=FALSE}
# Impute missing land use type from 2018 parcels (exclude no code and vacant)
########################
#pcl <- unique(bld.raw[, .(parcel_id, land_use_type_id)])
pcl[pcl2018, land_use_type_id_2018 := i.land_use_type_id, on = "parcel_id"]
impute <- (is.na(pcl$land_use_type_id) | pcl$land_use_type_id == 0) & !is.na(pcl$land_use_type_id_2018) & !pcl$land_use_type_id_2018 %in% c(16, 17, 26, 27)
```

`r if(join.with.2018parcels){paste("There are", sum(is.na(pcl$land_use_type_id) | pcl$land_use_type_id == 0, na.rm = TRUE), "parcels with land use type 0. After excluding 2018 parcels with no-code and vacant land use code,", sum(impute), "parcels can be imputed.")}`


```{r, eval=join.with.2018parcels, echo=FALSE}
if(sum(impute) > 0) {
  pcl[impute, `:=`(imp_land_use_type_id = TRUE, land_use_type_id = land_use_type_id_2018)]
  pcl[is.na(imp_land_use_type_id), imp_land_use_type_id := FALSE]
  orig.lut <- copy(bld$land_use_type_id)
  bld[pcl, `:=`(land_use_type_id_2018 = i.land_use_type_id_2018, land_use_type_id = i.land_use_type_id), on = "parcel_id_fips"]
}
```

`r if(join.with.2018parcels){paste(sum(orig.lut != bld$land_use_type_id, na.rm = TRUE), "buildings got their parcel's land use type changed.")} else {"No 2018-based imputation performed."}`
...
```{r, echo=FALSE}
consider <- with(bld, county_id %in% c(53, 61) & is_residential & building_type_id %in% c(12, 4) & !is.na(sqft_per_unit) & sqft_per_unit > 1 & residential_units > 1)
...
* all residential MF buildings with more than 1 unit with not-missing `sqft_per_unit` (`r format(sum(consider), big.mark = ",")`) that
...
There are `r nrow(bld[building_type_id %in% to.impute.bts])` buildings with missing building type (`r paste(to.impute.bts, collapse = ", ")`). Grouping them depending if they have residential units, non-residential sqft or both, we get the following table:
```{r, echo=FALSE}
bld[building_type_id %in% to.impute.bts, .(Nres = sum(is_residential), Nnonres = sum(is_non_residential),
                              Nmix = sum(is_mixuse))]
```
The remaining buildings of this type have no units and therefore cannot be grouped.
...
Here are results by counties (non-residential units are in thousands):
