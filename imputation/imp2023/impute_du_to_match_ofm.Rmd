...
#cb <- cb[, geoid20 := as.character(census_2020_block_id)]#[, census_2020_block_geoid := NULL]
...
#### Imputing SF into JBLM 

```{r, echo=FALSE}
# add SF buildings into JBLM (this step has been missed in the previous county-level steps)
fake.data.dir <- "~/psrc/urbansim-baseyear-prep/collect_parcels_buildings/BY2023/Pierce_data"
...
Imputed 1 SF building into each of the fake `r nrow(new)` JBLM parcels.
...
```{r, echo=FALSE}
bld[parcel_id_fips %in% jblm.pcl.id, sum(residential_units), by = "census_block_group_id"]
