...
block.lu <- block.lu[, geoid20 := as.character(census_2020_block_id)]#[, census_2020_block_geoid := NULL]

# merge with urbansim census tract ids
bg.lu <- fread(file.path(data.dir, "census_block_groups.csv"))
tracts.lu <- fread(file.path(data.dir, "census_tracts.csv"))
block.lu <- merge(merge(block.lu, bg.lu, by = "census_block_group_id"),
                  tracts.lu, by = "census_tract_id")

ofm <- merge(ofm, block.lu[, .(census_block_id, census_block_group_id, census_tract_id, county_id, geoid20, census_2020_block_group_id)], 
             by = c("geoid20", "county_id"), all.x = TRUE)
