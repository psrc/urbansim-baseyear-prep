...
```{r, echo=TRUE}
bld.file.name <- "imputed_buildings_20200316.csv"
...
```{r, echo=TRUE}
save.as.csv <- TRUE
out.file.name <- "imputed_buildings_ofm_match_20200316.csv"
```
...
# loac census blocks and create a block group dataset to get the right county_id
...
Here are the county totals. Column `DU` contains the final residential units. The columnn `DUimp1` are the imputed DUs after the first round of imputation. Thus, the amount imputed in this script is `DU - DUimp1`.

```{r, echo=FALSE}
fbydu[, .(DU = sum(DU), DUimp1 = sum(DUimp1), DUorig = sum(DUorig), HH = sum(HH)), by = county_id]
```

The regional total is:

```{r, echo=FALSE}
fbydu[, .(DU = sum(DU), DUimp1 = sum(DUimp1), DUorig = sum(DUorig), HH = sum(HH))]
```
