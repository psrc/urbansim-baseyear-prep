...
In the following tables, the DU column represent residential units that include imputation (column DUimp). Thus the original value for residential units can be derived as DU - DUimp. Column DUofm represent the OFM counts.
...
Here are block groups with the 15 largest positive differences:
...
Here are block groups with the 15 largest negative differences:

```{r, echo=FALSE}
head(aggdt[order(dif)], 15)
```

Here is a scatter plot of DU ~ DUofm for all block groups:
