...
Input files should be placed in the [imputation/data2018]( https://github.com/psrc/urbansim-baseyear-prep/blob/master/imputation/data2018) directory. Optionally, the 2014 parcels dataset can be used. The input datasets are:

* buildings.csv (name configurable)
* parcels.csv (name configurable)
* parcel_lookup_2018_2014.csv (optionally)
* imputation/data2014/parcels.csv (optionally)
...
Set the name of the buildings file (`bld.file.name`), if the resulting dataset should be written into a file (`save.as.csv`), the name of the resulting file (`out.file.name`) and if parcels should be joined with 2014 parcels to impute missing land use type (`join.with.2014parcels`):

```{r, echo=TRUE}
bld.file.name <- 'buildings.csv'
save.as.csv <- TRUE
out.file.name <- "imputed_buildings_20200616.csv"
join.with.2014parcels <- FALSE
```
...
`r if(join.with.2014parcels){paste("2014 parcels lookup table (file ", pcl14look.file.name, ") has been joint with ", pcl14.file.name, " and has ", nrow(pcl2014.look), " rows.")}`
...
```{r, eval=join.with.2014parcels, echo=FALSE}
...
`r if(join.with.2014parcels){paste("There are", sum(pcl$land_use_type_id == 0), "parcels with land use type 0. After excluding 2014 parcels with no-code and vacant land use code,", sum(impute), "parcels can be imputed.")}`
...
`r if(join.with.2014parcels){paste(sum(orig.lut != bld$land_use_type_id), "buildings got their parcel's land use type changed.")} else {"No 2014-based imputation performed."}`
...
tmp <- bld[change, .(N = .N, non_residential_sqft = sum(non_residential_sqft)), by = building_type_id]
bld[change, non_residential_sqft := 0]
```

* In Kitsap county, for some residential buildings we remove non-residential sqft: `r sum(tmp$non_residential_sqft)` sqft removed from `r sum(change)` buildings. 
...
Result: `r sum(consolidate)` buildings were collapsed into `r nrow(new_bld)` buildings. The new buildings table now has `r nbld.after` records. 
...
6. Impute missing `sqft_per_unit` in residential buildings:
    * for records with known units, known gross sqft and zero non-residential sqft, set it to `gross_sqft/residential_units` (`r sum(impute.from.gross)` records, covering a range from `r min(bld[impute.from.gross == TRUE, sqft_per_unit])` to `r max(bld[impute.from.gross == TRUE, sqft_per_unit])`);
...
`r sum(impute)` records were imputed. In the following plot the imputed points are in red.
...
```{r echo = FALSE}
reset <- with(bld, is.nonres & is.na(non_residential_sqft) & non_residential_sqft_orig > 0)
...
```{r echo = FALSE}
dt <- copy(bld)
s <- dt[, list(non_residential_sqft=round(sum(non_residential_sqft, na.rm=TRUE)/43560), residential_units=sum(residential_units, na.rm=TRUE)), by=county_id]
dtr <- data.table(bld.raw)
sr <- dtr[, list(non_residential_sqft=round(sum(non_residential_sqft, na.rm=TRUE)/43560), residential_units=sum(residential_units, na.rm=TRUE)), by=county_id]
...
Here are results by counties (non-residential units are in acres):
