...
save.as.csv <- TRUE
out.file.name <- "imputed_buildings_20200330.csv"
```
...
4. For single-use residential buildings we can impute gross sqft from improvement value. Here is the relationship (on the log scale):
...
5. Impute missing `sqft_per_unit` in residential buildings:
