...
Input data are csv files exported from the database 2018_parcel_baseyear_working. The main buildings table is an export of the buildings_for_imputation table, which was created from buildings and parcel attributes in mysql as follows:

```
create table buildings_for_imputation select a.parcel_id, building_id, 
    year_built, a.gross_sqft, non_residential_sqft, residential_units, 
    sqft_per_unit, stories, building_type_id, improvement_value, 
    a.county_id, land_use_type_id, parcel_sqft, tax_exempt, land_value, 
    is_inside_urban_growth_boundary 
  from buildings as a left join parcels as b on a.parcel_id=b.parcel_id;
```
...
* buildings_for_imputation.csv
* parcel_lookup_2018_2014.csv
* imputation/data2014/parcels.csv

The output is written into imputation/data2018/imputed_buildings.csv.
...
bld.file.name <- 'buildings_for_imputation.csv'
...
Other settings include specifying which are the residential building types, which types should be imputed and if the resulting dataset should be written into a file:
```{r, echo=TRUE}
...
In addition, we will remove residential units that are in buildings of the following types:
```{r, echo=TRUE}
...
tmp.cnty
```
In total `r sum(tmp$removed)` residential units removed.
...
* any of the buildings on a parcel have sqft per unit smaller than 300 (`r sum(consider & with(bld, Nmf_max > 1 & has_small_spu == TRUE  & residential_units >= Nmf_max))`) AND
...
There are `r nrow(bld[building_type_id %in% to.impute.bts])` buildings with missing building type. Groupping them depending if they have residential units, non-residential sqft or both, we get the follwing table:
```{r, echo=FALSE}
...
```{r, echo=FALSE}
impute <- with(bld, building_type_id %in% c(10, residential.bts) & is_non_residential)
bld[impute, building_type_id := 3]
```

* Reclassify buildings with residential or mix-use building type that only have non-residential sqft into commercial (type 3).  `r sum(impute)` records changed.

##### Results

In total, building_type_id was modified in `r sum(bld$building_type_id != bld$building_type_id_orig)` records. See the breakdown of the counts by the original and the resulting building type at the end of this document. A few records will change its building type in the next section.
...
5. For residential buildings we can impute gross sqft from improvement value. Here is the relationship on a log scale:
...
```
 ...
 10. Set group-quaters DUs to 0 (`r sum(imp)` records updated).
...
11. Set the remainder of missing residential units to 1 (`r sum(imp)` records updated).
 ...
 ```{r echo = FALSE}
file.out <- file.path(data.dir, "imputed_buildings.csv")
if(save.as.csv) {
