...
All input files are taken from the directory
```{r, echo=FALSE}
library(data.table)
...
if(save.as.csv) {
  ...
  write.table(bld, file=file.out, sep=',', row.names=FALSE)
}
```
...
In total, building_type_id was modified in `r sum(bld$building_type_id != bld$building_type_id_orig)` records.
