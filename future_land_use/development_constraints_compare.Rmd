...
far_diff_out_dotplot <- dotplot(far.out, "outliers") + labs(title = "MaxFAR outliers")
du_diff_out_dotplot <- dotplot(du.out, "outliers") + labs(title = "MaxDU outliers")

far_diff_out_dotplot + du_diff_out_dotplot & scale_y_continuous(NULL, breaks = NULL)
```

```{r, fig.width = 10, fig.width=10}
# View(df.du[max_du_diff %in% du.outliers$out])
du_diff_out_scatterplot <- scatterplot(df.du[max_du_diff %in% du.outliers$out], "Max_DU_Ac_prev", "imputed_value", "use_type", "source_value") +
  facet_wrap(Jurisdicti_new ~ .)
du_diff_out_scatterplot #+ 
  # geom_text(data = df.du[max_du_diff %in% du.outliers$out],
  #            aes(x = Max_DU_Ac_prev, y = imputed_value, label = str_extract(Key, "(?<=_).*")),
  #            size = 2,
  #           nudge_y = 1)
```
