...
hhsest <- hhs.rawsurvey[!is.na(final_home_parcel), .(hhid = hhid, puma = final_home_puma10,
                         ...
                         workers = numworkers, 
                         tenure = rent_own,
                         income_cat = hhincome_detailed, 
                         persons = hhsize,
                         move = ifelse(!is.na(prev_home_wa) & prev_home_wa %in% c(1,2), 1, 0),
                         sampling_weight = initial_expansion_factor_combined)]
...
for(icat in 1:(length(income.categories)-1)){
    ...
    hhsest[income_cat == icat, income := sample(income.values, nrow(hhsest[income_cat == icat]), replace = TRUE)]
}
